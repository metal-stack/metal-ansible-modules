...
FIND_QUERY = tenant_pb2.TenantServiceListRequest(
...
class TestMetalV2TenantModule(V2MetalModules):
    def setUp(self):
        super().setUp()
        import metal_v2_tenant

        self.module = metal_v2_tenant

    def test_present_create(self):
        self.interceptor = TestClientInterceptor([
            RpcCall(
                request=FIND_QUERY,
                response=tenant_pb2.TenantServiceListResponse(tenants=[]),
            ),
            RpcCall(
                request=tenant_pb2.TenantServiceCreateRequest(
                ...
                response=tenant_pb2.TenantServiceCreateResponse(
                    tenant=TEST_TENANT,
                ),
            ),
        ])
    ...
    def test_present_already_exists(self):
        self.interceptor = TestClientInterceptor([
            RpcCall(
                request=FIND_QUERY,
                response=tenant_pb2.TenantServiceListResponse(
                    tenants=[TEST_TENANT],
                ),
            ),
        ])
    ...
    def test_present_update(self):
        ...
        updated_tenant.name = "my-tenant-updated"
